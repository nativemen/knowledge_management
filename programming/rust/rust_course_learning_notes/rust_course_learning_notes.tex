...
\section{不错的\textbf{unsafe}队列}{
    ...
    \subsection{栈借用}{
        ...
        \fact{
            \textbf{Miri}还提供了试验性的模式: "-Zmiri-tag-raw-pointers"。可以通过环境的方式来开启该模式:\\
            \texttt{MIRIFLAGS="-Zmiri-tag-raw-pointers" cargo +nightly miri test}\\
            如果是\textbf{Windows}，你需要设置全局变量:\\
            \texttt{\$env:MIRIFLAGS="-Zmiri-tag-raw-pointers"}\\
            \texttt{cargo +nightly miri test}
        }
