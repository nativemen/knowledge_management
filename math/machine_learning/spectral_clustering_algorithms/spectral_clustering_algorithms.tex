...
\section{Matrices}{
    ...
    \pf{
        Extend the dot product to complex vectors by $\langle\mathbf{v},\mathbf{w}\rangle=\sum_{i=1}^{n}\overline{\mathbf{v}_{i}}\mathbf{w}_{i}$, where $\overline{\mathbf{v}_{i}}$ is the complex conjugate of $\mathbf{v}_{i}$.\\
        ...
        \begin{align}
            \langle\mathbf{A}\mathbf{v},\mathbf{w}\rangle
                &=\sum_{i=1}^{n}\overline{(\mathbf{A}v)_{i}}w_{i}\\
                &=\sum_{i=1}^{n}\overline{\left(\sum_{j=1}^{n}\mathbf{A}_{ij}v_{j}\right)}w_{i}\\
                &=\sum_{i=1}^{n}\sum_{j=1}^{n}\overline{\mathbf{A}_{ij}\mathbf{v}_{j}}\mathbf{w}_{i}
        \end{align}
        Since $\mathbf{A}$ is a real matrix, then $\mathbf{A}_{ij}$ is real, therefore $\overline{\mathbf{A}_{ij}}=\mathbf{A}_{ij}$.\\
        We can write the $j$-th component of the product of matrix $\mathbf{A}$ and vector $\mathbf{w}$ as:
        \[\left(\mathbf{A}\mathbf{w}\right)_{j}=\sum_{i=1}^{n}\mathbf{A}_{ij}\mathbf{w}_{i}\]
        When we compute the $j$-th component of the product of matrix $\mathbf{A}^{\mathbf{T}}$ and vector $\mathbf{w}$, we get:
        \[\left(\mathbf{A}^{\mathbf{T}}\mathbf{w}\right)_{j}=\sum_{i=1}^{n}\left(\mathbf{A}^{\mathbf{T}}\right)_{ji}\mathbf{w}_{i}\]
        ...
        \begin{align}
            \langle\mathbf{A}\mathbf{v},\mathbf{w}\rangle
                &=\sum_{i=1}^{n}\sum_{j=1}^{n}\overline{\mathbf{A}_{ij}\mathbf{v}_{j}}\mathbf{w}_{i}\\
                &=\sum_{i=1}^{n}\sum_{j=1}^{n}\mathbf{A}_{ij}\overline{\mathbf{v}_{j}}\mathbf{w}_{i}\\
                &=\sum_{j=1}^{n}\overline{\mathbf{v}_{j}}\left(\sum_{i=1}^{n}\mathbf{A}_{ij}\mathbf{w}_{i}\right)\\
