...
\subsection{Diagonal matrix}{
    ...
    \cor{
        A diagonal matrix $\mathbf{D}_{n \times n}$ is invertible if all its diagonal elements are non-zero. The inverse of a diagonal matrix is also a diagonal matrix, with each diagonal element being the reciprocal of the original diagonal element.
        \[
    ...
    \defn{Weighted vertex-edge matrix}{
        ...
        \[
            \mathbf{e}(\tau)=
                ...
                \right.
        \]
        Here, $\mathbf{e}(\tau)$ denotes the $\tau$-th value of the edge vector $\mathbf{e}$. For undirected graphs, the negative endpoint can be either at the $\tau=i$ location or at the $\tau=j$ location. Let $\mathbf{E}_{n \times m}=[\mathbf{e}_{0},\cdots,\mathbf{e}_{m-1}]$
...
\subsection{Normalized vertex-edge matrix}{
    \defn{Normalized vertex-edge matrix}{
        Let $\mathbf{D}$ be the Degree matrix of the graph with the $i$-th diagonal element:
        \[d_{i}=\sum_{j=1}^{n}w_{ij}\]
        Let $\mathcal{E}=[\epsilon_{0},\cdots,\epsilon_{m-1}]$ be the normalized vertex-edge matrix with each $n$-dimensional edge $\epsilon$ vector normalized:
        $\epsilon=\sqrt{w_{ij}}[0,\cdots,0,\sqrt{d_{i}},0,\cdots,0,-\sqrt{d_{i}},0,\cdots,0]^{\mathbf{T}}$
        where the $i$-th location is $\sqrt{d_{i}}$, the $j$-th location is $-\sqrt{d_{i}}$, and other locations are 0.
        \[
            \epsilon(\tau)=
                \left\{
                    \begin{array}{ll}
                        d_{i}^{-\frac{1}{2}}w_{ij}^{\frac{1}{2}}  & \text{if }\tau\text{ is }$i$,\\
                        -d_{i}^{-\frac{1}{2}}w_{ij}^{\frac{1}{2}} & \text{if }\tau\text{ is }$j$,\\
                        0                                        & \text{otherwise}
                    \end{array}
                \right.
        \]
    ...
    \label{Normalized vertex-edge matrix}
}
...
\subsection{Laplacian matrix}{
    ...
    \cor{
        Let $\mathbf{E}_{n \times m}=[\mathbf{e}_{0},\cdots,\mathbf{e}_{m-1}]$ be the $n \times m$ weighted vertex-edge matrix in the an undirected graph of $n$ vertices and $m$ edges (\ref{Weighted vertex-edge matrix}).\\
        The graph Laplacian matrix can be computed as follows
        \[\mathbf{L}_{n \times n}=\mathbf{E}\mathbf{E}^{\mathbf{T}}=\sum_{t=0}^{m-1}\mathbf{e}_{t}\mathbf{e}_{t}^{\mathbf{T}}\]
    }
    \pf{
        Since $\mathbf{E}_{n \times m}=[\mathbf{e}_{0},\cdots,\mathbf{e}_{m-1}]$ be the $n \times m$ weighted vertex-edge matrix in the an undirected graph of $n$ vertices and $m$ edges.\\
        ...
        \begin{align}
            \mathbf{E}_{n \times m}\mathbf{E}_{n \times m}^{\mathbf{T}}
                &=
                \begin{bmatrix}
                    \mathbf{e}_{0} & \cdots & \mathbf{e}_{m-1}
                \end{bmatrix}
                \begin{bmatrix}
                    \mathbf{e}_{0}^{\mathbf{T}}\\
                    \vdots\\
...
\subsection{Normalized Laplacian matrix}{
    \defn{Symmetrically normalized Laplacian matrix}{
        The symmetrically normalized graph Laplacian matrix can be computed as:
    ...
    \cor{
        \[\mathcal{L}_{sym}=\mathcal{E}\mathcal{E}^{\mathbf{T}}=\sum_{t=0}^{m-1}\mathbf{\epsilon}_{t}\mathbf{\epsilon}_{t}^{\mathbf{T}}\]
    }
}
